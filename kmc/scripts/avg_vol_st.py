...
data = [loadtxt(mypopdir + 'rmsvol_st_seed=%d.txt' % (i+1)) for i in range(ntraj)]

#short-time data
xmin = min(line[1:,0].min() for line in data)
xmax = max(line[1:,0].max() for line in data)

x_points = linspace(log10(xmin), log10(xmax), 1000)
...
savetxt(mypopdir + 'rmsvol_avg_st.txt', c_[x_points, averages, stdevs])
