...
//mimicking cation exchange in a nanocrystal.
//
...
#define Z 6 //Change for different lattice structures
...
int main(int argc, char *argv[]){
  ...
  if(argc != 10){
    printf("Error: require seven arguments (is_arr_job, Ns, kT, h, k_swap_pow, J, nsweep, init, is_no_ex.)\n");
    exit(-1);
  }
  else{
    ...
    kT = atof(argv[3]);
    h = atof(argv[4]);
    k_swap_pow = atof(argv[5]);
    J = atof(argv[6]);
		nsweep = atoi(argv[7]);
		init = argv[8];
		is_no_ex = atoi(argv[9]);
  }
  ...
  Nlat = 3*Ns*Ns - 3*Ns + 1; //for hexagon
  Nsurf = 6*(Ns-1); //for hexagon
  ...
  char input[1000];
  sprintf(input, "../nc_matrices/veff_vec_Ns=%d.txt", Ns);
...
double run_traj_elastic(std::vector<atom> &lat, double *fraction, double *time,
  ...
  double t_curr = 0;
    ...
    //  printf("%f ", partsum[i]-partsum[i-1]);
    //}
    //printf("\n");
    ...
    if(t<Nlat){
      std::cout << "Pass number " << t << "\n";
	    //Check if directory exists
		  char dir[200];
		  sprintf(dir, "data/init=%s/J=%f/Ns=%d/kT=%f/h=%f/k_swap=1e%d/confs/seed=%03d/", init_short.c_str(), J, Ns, kT, h, k_swap_pow, myseed);
		  ...
		  char outp[200];
		  sprintf(outp, "data/init=%s/J=%f/Ns=%d/kT=%f/h=%f/k_swap=1e%d/confs/seed=%03d/shortt_%04d.xyz", init_short.c_str(), J, Ns, kT, h, k_swap_pow, myseed, t);
    ...
    if(t%print_freq == 0){
      std::cout << "Pass number " << t/Nlat << "\n";
	    //Check if directory exists
		  ...
		  char outp[200];
		  sprintf(outp, "data/init=%s/J=%f/Ns=%d/kT=%f/h=%f/k_swap=1e%d/confs/seed=%03d/conf_%04d.xyz", init_short.c_str(), J, Ns, kT, h, k_swap_pow, myseed, t/print_freq);
		  pconf(lat, outp, t_curr);
    }

//    int found_good_event=0;
//    while(found_good_event==0){
...
/*
	else if(boost::algorithm::ends_with(init_conf, ".xyz")){
		std::cout << "Reading in spins from file." << std::endl;
	}
	else{
		std::cout << "Error: init not recognized." << std::endl;
		exit(-1);
	}
*/
	...
	for(int i=0; i<Nlat; i++){
		...
		for(int j=0; j<Nlat; j++){
			if(i!=j){
				if(get_dist(lat[i], lat[j])<(lavg+1e-4)){
          if(j>i) Npair++;
...
void print_pop(double *time, double *fraction, int nstp){
  ...
  //Check if directory exists
  char dir[200];
  sprintf(dir, "data/init=%s/J=%f/Ns=%d/kT=%f/h=%f/k_swap=1e%d/pops/", init_short.c_str(), J, Ns, kT, h, k_swap_pow);
  struct stat fileStat;
  ...
  //Write file
	FILE *f;
  char outp[200];
  sprintf(outp, "data/init=%s/J=%f/Ns=%d/kT=%f/h=%f/k_swap=1e%d/pops/kmc_pop_seed=%lu.txt", init_short.c_str(), J, Ns, kT, h, k_swap_pow, myseed);
  f = fopen(outp, "w");
