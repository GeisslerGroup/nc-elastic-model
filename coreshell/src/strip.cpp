//Perform a Metropolis Monte Carlo simulation
//mimicking cation exchange in a strip geometry.
...
int main(int argc, char *argv[]){
  ...
  if(argc != 9){
    printf("Error: require eight arguments (field, temperature, nsteps, net_c, nx, ny, nlayer, seed.)\n");
    exit(-1);
  }
  else{
    h = atof(argv[1]);
  ...
  char input[1000];
  sprintf(input, "/home/layne/research/ionex/strip_matrices/veff_vec_nx=%d_ny=%d.txt", nx, (ny+2*nlayer));
  ...
  run_mc_equil(lat, nequil, veff, dynamics);
	...
	//Run MC
  char confdir[200];
  sprintf(confdir, "confs/strip/kT=%f/h=%f/nx=%d_ny=%d/c=%f/nlayer=%d/", kT, h, nx, ny, net_c, nlayer);
  ...
  char dir[200];
  sprintf(dir, "pops/strip/kT=%f/h=%f/nx=%d_ny=%d/c=%f/nlayer=%d/", kT, h, nx, ny, net_c, nlayer);
...
double run_mc_equil(std::vector<atom> &lat, int nstp, arma::mat veff, char *dynamics){
    ...
    char outp[300];
    sprintf(outp, "../../phase_sep/strip/confs/equil_%04d.xyz", t);
    pconf(lat, outp, t);
    //Do passes
    for(int i=0; i<Ncore; i++){
      if(strcmp(dynamics, "metropolis")==0){
        ...
        while((int)m1<nx*nlayer || (int)m1>(Nlat-nlayer*nx-1)) m1 = gsl_rng_uniform_int(rg,(unsigned long int)Nlat);
        unsigned long int m2 = gsl_rng_uniform_int(rg,(unsigned long int)Nlat);
        while((int)m2<nx*nlayer || (int)m2>(Nlat-nlayer*nx-1)) m2 = gsl_rng_uniform_int(rg,(unsigned long int)Nlat);
