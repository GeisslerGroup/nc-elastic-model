...
int main(int argc, char *argv[]){
  ...
  if(argc!=2){
    printf("Error: need size of hexagon.\n");
    exit(-1);
  }
  ...
  Ns = atoi(argv[1]);
	...
	Nlat = 3*Ns*Ns - 3*Ns + 1; //See notebook for derivation
  ...
  for(int i=0; i<Nlat; i++){
    ...
    for(int k=0; k<lat[i].n; k++){
      dyn(2*i,2*i) += alpha_mat(abs(diagonal[i][k+1])-1, 0)*alpha_mat(abs(diagonal[i][k+1])-1, 0);
      dyn(2*i,2*i+1) += alpha_mat(abs(diagonal[i][k+1])-1, 0)*alpha_mat(abs(diagonal[i][k+1])-1, 1);
      dyn(2*i+1,2*i) += alpha_mat(abs(diagonal[i][k+1])-1, 1)*alpha_mat(abs(diagonal[i][k+1])-1, 0);
      dyn(2*i+1,2*i+1) += alpha_mat(abs(diagonal[i][k+1])-1, 1)*alpha_mat(abs(diagonal[i][k+1])-1, 1);
    }
    for(int j=0; j<Nlat; j++){
      if(i!=j && is_neighbor(lat, i, j)){
        dyn(2*i, 2*j) = -alpha_mat(abs(off_diag[i][j])-1, 0)*alpha_mat(abs(off_diag[i][j])-1, 0);
        dyn(2*i, 2*j+1) = -alpha_mat(abs(off_diag[i][j])-1, 0)*alpha_mat(abs(off_diag[i][j])-1, 1);
        dyn(2*i+1, 2*j) = -alpha_mat(abs(off_diag[i][j])-1, 1)*alpha_mat(abs(off_diag[i][j])-1, 0);
        dyn(2*i+1, 2*j+1) = -alpha_mat(abs(off_diag[i][j])-1, 1)*alpha_mat(abs(off_diag[i][j])-1, 1);
      }
    }
    //Coupling Matrix
    for(int k=0; k<lat[i].n; k++){
      coup(2*i, i) += alpha_mat(abs(diagonal[i][k+1])-1, 0)*copysign(1.0, -diagonal[i][k+1]);
      coup(2*i+1, i) += alpha_mat(abs(diagonal[i][k+1])-1, 1)*copysign(1.0, -diagonal[i][k+1]);
    }
    for(int j=0; j<Nlat; j++){
      if(i!=j) ising(i,j) = -4.0/Nlat;
      if(i!=j && is_neighbor(lat, i, j)){
        coup(2*i, j) += alpha_mat(abs(off_diag[i][j])-1, 0)*copysign(1.0, -off_diag[i][j]);
        coup(2*i+1, j) += alpha_mat(abs(off_diag[i][j])-1, 1)*copysign(1.0, -off_diag[i][j]);
        //ising(i,j) += 0.01;
      }
    }
    //Full Matrix
 ...
 // std::cout << accu(veff*rotkin - rotkin*veff) << std::endl;
...
void boys_min(std::vector<site> &lat, arma::mat eigvec, arma::vec coeffs){
  ...
  arma::mat rsq(num_surf, num_surf, arma::fill::zeros); 
  arma::mat rx(num_surf, num_surf, arma::fill::zeros);
  arma::mat ry(num_surf, num_surf, arma::fill::zeros);
  for(int i=0; i<num_surf; i++){
   for(int j=0; j<num_surf; j++){
     for(int r=0; r<Nlat; r++){
       rsq(i,j) += evecs(r,i)*evecs(r,j)*(lat[r].x*lat[r].x+lat[r].y*lat[r].y);
       rx(i,j) += evecs(r,i)*evecs(r,j)*lat[r].x;
       ry(i,j) += evecs(r,i)*evecs(r,j)*lat[r].y;
     }
   }
  } 
...
void pbonddata(int **d, int **od){
  ...
  FILE *f;
  char outp1[200];
  if(strcmp(shape, "hexagon")==0) sprintf(outp1, "nc_matrices/%s/topology_diag_Ns=%d.txt", shape, Ns);
  ...
  FILE *f2;
  char outp2[200];
  if(strcmp(shape, "hexagon")==0) sprintf(outp2, "nc_matrices/%s/topology_offdiag_Ns=%d.txt", shape, Ns);
...
void pmatrices(arma::mat dyn, arma::mat coup, arma::mat spin, arma::mat veff){
  ...
  FILE *f;
  char outp1[200];
  if(strcmp(shape, "hexagon")==0)  sprintf(outp1, "nc_matrices/%s/dyn_mat_Ns=%d.txt", shape, Ns);
  ...
  if(strcmp(shape, "hexagon")==0)  sprintf(outp1, "nc_matrices/%s/coup_mat_Ns=%d.txt", shape, Ns);
  ...
  if(strcmp(shape, "hexagon")==0)  sprintf(outp1, "nc_matrices/%s/spin_mat_Ns=%d.txt", shape, Ns);
  ...
  if(strcmp(shape, "hexagon")==0)  sprintf(outp1, "nc_matrices/%s/veff_mat_Ns=%d.txt", shape, Ns);
  ...
  if(strcmp(shape, "hexagon")==0)  sprintf(outp1, "nc_matrices/%s/veff_vec_Ns=%d.txt", shape, Ns);
...
void pevals(arma::vec evals){
  ...
  FILE *f;
  char outp[300];
  if(strcmp(shape, "hexagon")==0)  sprintf(outp, "nc_matrices/%s/evals_Ns=%d.txt", shape, Ns);
...
void pevecs(arma::mat evecs, std::vector<site> &lat){
  ...
  for(int i=0; i<Nlat; i++){
    FILE *f;
    char outp[300];
    if(strcmp(shape, "hexagon")==0) sprintf(outp, "nc_matrices/evecs/%s/evec_Ns=%d_%d.txt", shape, Ns, i);
...
void pfonevals(arma::vec evals){
  ...
  FILE *f;
  char outp[300];
  if(strcmp(shape, "hexagon")==0)  sprintf(outp, "nc_matrices/%s/fonevals_Ns=%d.txt", shape, Ns);
...
void pconfigs(std::vector<site> &lat, arma::mat veff){
  ...
  for(int i=0; i<Nlat; i++){
    FILE *f;
    char outp[300];
    if(strcmp(shape, "hexagon")==0)  sprintf(outp, "nc_matrices/pair_configs/%s/atom_%d_Ns=%d.txt", shape, i, Ns);
